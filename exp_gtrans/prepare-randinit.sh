...
# data builder
if [ $input == "doc" ]; then
  python3 -m exp_gtrans.data_builder --datadir $tok_path --destdir $seg_path/ --source-lang $slang --target-lang $tlang --max-tokens 512 --max-sents 1000  --divided-group-sentences $group_sentences
elif [ $input == "sent" ]; then
  python3 -m exp_gtrans.data_builder --datadir $tok_path --destdir $seg_path/ --source-lang $slang --target-lang $tlang --max-tokens 512 --max-sents 1  --divided-group-sentences $group_sentences
fi
