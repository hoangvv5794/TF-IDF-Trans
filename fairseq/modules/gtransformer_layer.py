    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if key_global in state_dict and state_dict[key_global]:
            for old, new in attn_name_map.items():
                for m in ["in_proj_weight", "in_proj_bias", "out_proj.weight", "out_proj.bias",
                          "k_proj.weight", "k_proj.bias", "q_proj.weight", "q_proj.bias", "v_proj.weight",
                          "v_proj.bias"]:
...
class GTransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.self_attn_local = self.build_self_attention(self.embed_dim, args, add_bias_kv=add_bias_kv,
        ...
        if dec_add_global_attn:
            self.self_attn_global = self.build_self_attention(self.embed_dim, args, add_bias_kv=add_bias_kv,
                                                              add_zero_attn=add_zero_attn)
            self.self_attn_gate = nn.Sequential(Linear(self.embed_dim * 2, self.embed_dim), nn.Sigmoid())
