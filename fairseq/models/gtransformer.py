...
# Hoang Vu: changed on 2023/12/26
...
class GTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.tf_idf_mode = args.tf_idf_mode == 'enable'
        self.tf_idf_score = args.tf_idf_score
    ...
    def forward(self, src_tokens, src_lengths, return_all_hiddens: bool = False):
        ...
        # Vu Hoang: compute tf-idf score of list sentences in source and make new Masked Attention,
        # Grouping list sentences has similarity over 0.8
        if self.partial_mode:
            # compute attention mask
            local_attn_mask = src_tags.unsqueeze(1) != src_tags.unsqueeze(2)
            local_attn_mask &= 0 != src_tags.unsqueeze(2)
            # if enable mode TF-IDF
            if self.tf_idf_mode:
                # divide src_tokens to each tensor to make corpus
                unique_mask = torch.unique(local_attn_mask, dim=1)
                final_output = []
                for i, x in enumerate(unique_mask.squeeze(0)):
                    element = src_tokens.squeeze(0)[x]
                    final_output.append(element)
                corpus = (torch.stack(final_output)).float()
                print('Corpus shape: ' + corpus.shape)
                # adding tf-idf score to group sentence has score > 0.2
                x_cosine_similarity = F.cosine_similarity(corpus[None, :, :], corpus[:, None, :], dim=-1)
                # This should print the same matrix as above.
                print('Matrix cosine: ' + x_cosine_similarity)
                # Make a mask_tag has score over 0.82
                mask_tag = (x_cosine_similarity > self.tf_idf_score)
                length_sentence = corpus.shape[1]
                new_local_attn_mask = mask_tag.repeat_interleave(length_sentence, dim=1).repeat_interleave(
                    length_sentence,
                    dim=0)
                local_attn_mask = new_local_attn_mask
        # encoder layers
        attn = []
